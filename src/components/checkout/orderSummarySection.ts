...
// 
  const discountInPercent = ((data.total - data.discountedTotal)/ data.total*100).toFixed(2);
    ...
    const itemSubtotal = createItem('Subtotal', 'grayStyle', '$'+data.total.toString(), 'blackBoldText');
    ...
    if (discountInPercent > 0) {
        ...
        const total = parseFloat((data.total*(100-discountInPercent)/100).toFixed(2));
        ...
        const itemTotal = createItem('Total', 'blackStyle', '$'+data.total.toString(), 'blackBigBoldText');
